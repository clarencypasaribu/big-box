        ...
        parsed.forEach((column) => {
          ...
          column.cards.forEach((card) => {
            next.push({
              ...
              stage: column.title,
              done: card.done || column.status === "Completed",
            });
          });
        });
      } catch {
    ...
    <Card className="border-slate-200 shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button asChild variant="ghost" size="sm" className="text-slate-600">
          <Link href="/member/projects/mobile">
            View project board
        ...
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <Link
              ...
              className={cn(
                "relative flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-slate-300 hover:shadow-md",
            ...
            >
              ...
              <div className="relative z-10 flex items-start justify-between gap-2">
                ...
                <div className="flex flex-col items-end gap-1 text-xs">
                  ...
                  <Badge variant="outline" className={cn("rounded-md", statusTone[task.status])}>
                    {task.status}
                  </Badge>
                </div>
              </div>
